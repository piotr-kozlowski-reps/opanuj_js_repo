...
function useEmoji(input){
  ...
  for(const [key, value] of Object.entries(emojiMappings)){
    ...
    const actualRegexAsString = "(?<!\\w)" + key + "(?!\\w)";
    const actualRegex = new RegExp(actualRegexAsString, "gi");
    resultString = resultString.replaceAll(actualRegex, emojiMappings[key]);
  }
