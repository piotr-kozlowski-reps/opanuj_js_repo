...
function totalDuration() {
  ...
  const allCoursesInSeconds = returnDurationOfAllCourses();
  const allCoursesData = courseInfoData(allCoursesInSeconds, allCoursesInSeconds)
  
  const firstCourseInSeconds = returnDurationOfCourseInSeconds(1);
  const firstCoursedata = courseInfoData(firstCourseInSeconds, allCoursesInSeconds);
  
  const secondCourseInSeconds = returnDurationOfCourseInSeconds(2);
  const secondCoursedata = courseInfoData(secondCourseInSeconds, allCoursesInSeconds);

  const thirdCourseInSeconds = returnDurationOfCourseInSeconds(3);
  const thirdCoursedata = courseInfoData(thirdCourseInSeconds, allCoursesInSeconds);
  
  
  return `Kurs Opanuj JavaScript trwa ${allCoursesData.hours} godzin i ${allCoursesData.minutes} minut.\n
  Moduł Adama: ${firstCoursedata.hours} godzin(y) ${firstCoursedata.minutes} minut (${firstCoursedata.percentageOfTotal} % całości kursu)\n
  Moduł Przemka: ${secondCoursedata.hours} godzin(y) ${secondCoursedata.minutes} minut (${secondCoursedata.percentageOfTotal} % całości kursu)\n
  Moduł Marcina: ${thirdCoursedata.hours} godzin(y) ${thirdCoursedata.minutes} minut  (${thirdCoursedata.percentageOfTotal} %) całości kursu`;
}



//utils
function returnDurationOfCourseInSeconds(courseNumber) {

  let totalLenghtOfAllCoursesInSeconds = 0;

  for (let i = 0; i < lessons.length; i++){

    let isMatchingCourseNumber = checkMatchingCourseNumber(lessons[i], courseNumber);

    if(isMatchingCourseNumber) {
      const timeSplitedAsStrings = lessons[i]['length'].split(":");
      totalLenghtOfAllCoursesInSeconds += Number.parseInt(timeSplitedAsStrings[0]) * 60;
      totalLenghtOfAllCoursesInSeconds += Number.parseInt(timeSplitedAsStrings[1]);
    }
  }

  return totalLenghtOfAllCoursesInSeconds;
}

function returnDurationOfAllCourses(){
...
function checkMatchingCourseNumber(objectToCheck, courseNumber){

  const regex = /(?<=\[)([\s\S]*?)(?=x)/;
  const foundCourseNumberAsArray = regex.exec(objectToCheck.name);

  if(foundCourseNumberAsArray === null) return false;
  const foundCourseNumber = Number.parseInt(foundCourseNumberAsArray[0]);    

  return (foundCourseNumber === courseNumber) ? true : false;
}

function courseInfoData(durationOfCourseInSeconds, totalDuration){
  return {
    hours: Math.floor(durationOfCourseInSeconds / 60 / 60),
    minutes: durationOfCourseInSeconds% 60,
    percentageOfTotal: (durationOfCourseInSeconds / totalDuration * 100).toFixed(2),
  };
}
